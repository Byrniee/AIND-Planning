...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if actionNode.prenodes.issubset(self.s_levels[level]):
                ...
                for literalNode in self.s_levels[level]:
                    literalNode.children.add(actionNode)
                    actionNode.parents.add(literalNode)
    ...
    def add_literal_level(self, level):
        ...
        for actionNode in self.a_levels[level - 1]:
            for literalNode in actionNode.effnodes:
                literalNodes.add(literalNode)
                
                literalNode.parents.add(actionNode)
                actionNode.children.add(literalNode)
        
        self.s_levels.append(literalNodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for addEffect in node_a1.action.effect_add:
            if addEffect in node_a2.action.effect_rem:
                return True

        for addEffect in node_a2.action.effect_add:
            if addEffect in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for addEffect in node_a1.action.effect_add:
            if addEffect in node_a2.action.precond_neg:
                return True

        for addEffect in node_a2.action.effect_add:
            if addEffect in node_a1.action.precond_neg:
                return True


        for remEffect in node_a1.action.effect_rem:
            if remEffect in node_a2.action.precond_pos:
                return True

        for remEffect in node_a2.action.effect_rem:
            if remEffect in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precondA1 in node_a1.parents:
            for precondA2 in node_a2.parents:
                if precondA1.is_mutex(precondA2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precondS1 in node_s1.parents:
            for precondS2 in node_s2.parents:
                if not precondS1.is_mutex(precondS2):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        """
        levelSum = 0
        # TODO For each goal in the problem, determine the level cost, then add them together
